    ...
    if([aImageUrls count] > 0) {
        ...
        for (int i = 0; i < [aImageUrls count]; i++) {
            ...
            if([[aImageUrls objectAtIndex:i] isKindOfClass:[UIImage class]]) {
                ...
                __weak UIImageView *mImageView = imageView;
                ...
                [imageView setImageWithURLRequest:request placeholderImage:[UIImage imageNamed:@"Image_Placeholder"] success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
                    
                    [mImageView setImage:image];
                        [mImageView setContentMode:[_dataSource contentModeForImage:i inPager:self]];
                    //
